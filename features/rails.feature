...
# 
# The application must be implemented in latest Rails 4.2.x
...
# This feature must be runnable by cucumber
...
# Sanity cheks must run after every commit to github (rubocop). Code must be compatible with the thoughbot standards
# Application can be deployed to the appserver with capistrano
# If you create Ansible playbooks to deploy this app and setup server stack in AWS cloud EC2 then this is a big plus! 
